...
def extract_pupil_from_stack(pupil_stack,thrsh = 'auto'):
    ...
    centre_ix = np.array(pupil_stack[0].shape)/2
...
def write_pupil_video(write_path,pupil,pupil_fit):
    ...
    for i in range(len(pupil_fit)):
        ...
        #if i>2:
            #writeIm = cv2.polylines(writeIM, [pts.T[:i]], 0, [255,0,0])
