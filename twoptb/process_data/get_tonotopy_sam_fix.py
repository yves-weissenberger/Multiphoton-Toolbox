...
objective_multiplier = 1000 #FOV of your objective in microns (at zoom level 1) (16x=1000 20x=800)
zoom = 1 # zoom-level

zoom_multiplier = 512*zoom
real_fov = objective_multiplier/zoom
#if len(sys.argv>2):
...
def get_tuning_curves(areaF,centre=None):
    ...
    xPos = (roi_centres[:,1]*objective_multiplier/zoom_multiplier) + (FOV_centre[0] - 0.5*real_fov)
    yPos = (-roi_centres[:,0]*objective_multiplier/zoom_multiplier) + (FOV_centre[1] + 0.5*real_fov)
    ...
    all_resps = np.zeros([n_neurons,
                          ...
                          int(math.ceil(len(outDat['stimOrder'])/float(outDat['stim_list'].shape[0])))])
    ...
    for neuron in range(n_neurons):
        ...
        win=np.array([2,10])
        win_bl=win[0]+np.array([-5,-1])
        #print stimSP
        stim_counter = np.zeros([len(np.unique(outDat['stim_list']))],dtype='int')
        for idx,stim in enumerate(outDat['stimOrder']):
            #idx=idx+1 ### TAKE THIS OUT!! ##
            if idx>0:
                resps[neuron,stim-1] += np.mean(use_trace[(idx*stimSP)+win[0]:(idx*stimSP)+win[1]]) - np.mean(use_trace[(idx*stimSP)+win_bl[0]:(idx*stimSP)+win_bl[1]])
                all_resps[neuron,stim-1,int(stim_counter[stim-1])] = np.mean(use_trace[(idx*stimSP)+win[0]:(idx*stimSP)+win[1]]) - np.mean(use_trace[(idx*stimSP)+win_bl[0]:(idx*stimSP)+win_bl[1]])
            else:
                resps[neuron,stim-1] += np.mean(use_trace[(idx*stimSP)+win[0]:(idx*stimSP)+win[1]])
                all_resps[neuron,stim-1,int(stim_counter[stim-1])] = np.mean(use_trace[(idx*stimSP)+win[0]:(idx*stimSP)+win[1]])

            stim_counter[stim-1] += 1

        for idx,stim in enumerate(np.unique(outDat['stimOrder'])): # deal with missing values by adding the mean (DODGY)
            if stim_counter[stim-1] < np.ceil(np.mean(stim_counter)):
                all_resps[neuron,stim-1,int(stim_counter[stim-1])] = np.mean(all_resps[neuron,stim-1])
                stim_counter[stim-1] += 1
    ...
    good = ((np.max(resps,axis=1) - np.mean(resps,axis=1))/np.mean(resps,axis=1))>3 # the old way
...
for area in areas:
    ...
    areaF = tonemap[area]
    if ('Tono' in area or 'area' in area or 'Area' in area or "zoom1" in area):
