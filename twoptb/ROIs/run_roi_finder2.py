...
=============================================================================""",
    ...
    parser.add_argument("-conf",type=int,dest='conf',default=1,
                help="Confirm ROI Masks")
    ...
    if args.conf=="0" or args.conf==0:
        confirm_rois = 0
        print("Warning will overwrite previously drawn ROIs without confirmation")
    else:
        confirm_rois = 1


    if areaSpec==-1:
        area_set = range(len(areas))
    else:
        area_set = [areaSpec]
    ...
    for areaIx in area_set:
        print areaIx
        print "extracting from %s" %(areas[areaIx])

        meanIm11 = bhdf11[areas[areaIx]].attrs['corr_image']
        ...
        plt.imshow(bhdf11[areas[areaIx]].attrs['mean_image'] ,cmap='binary_r')
        plt.imshow(mask_fin)
